...
def test_t1w_dcm_volume_modality(mock_t1_series):
    for series in mock_t1_series:
    ...
    #     for volume in series.get_volume_list():
    #         # assert volume.get_modality() == "adc"
    #         if 12 <= series_num <= 15:
    #             assert volume.get_modality() == "t1w"


def test_ax_dcm_volume_acq_plane(mock_ax_series):
...
def test_sag_dcm_volume_acq_plane(mock_sag_series):
    for series in mock_sag_series:
        for volume in series.get_volume_list():
...
def test_cor_dcm_volume_acq_plane(mock_cor_series):
    for series in mock_cor_series:
        for volume in series.get_volume_list():
