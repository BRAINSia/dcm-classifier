...
# df = pd.read_excel(
#     "/home/mbrzus/programming/dcm_train_data/V2-20240229/no_duplicates/combined_all_data_Feb29.xlsx"
# )
#
# columns_to_keep = [
#     "FileName",
...
# new_df.to_excel(
#     "/home/mbrzus/programming/dcm_train_data/V2-20240229/no_duplicates/labeling/combined_all_data_labeling_Feb29.xlsx"
# )

# # get all unique series descriptions
# df = pd.read_excel(
#     "/home/mbrzus/programming/dcm_train_data/V2-20240229/no_duplicates/labeling/combined_all_data_labeling_Feb29_remaining.xlsx"
# )
# series_descriptions = df["SeriesDescription"].unique()
# new_df = pd.DataFrame(series_descriptions, columns=["Series Description"])
# new_df.to_excel(
#     "/home/mbrzus/programming/dcm_train_data/V2-20240229/no_duplicates/labeling/unique_series_descriptions_remaining.xlsx"
# )
...
def update_labels_and_series_desc(series_desc_list: list, label: str):
    ...
    final_df = pd.read_excel(
        "/home/mbrzus/programming/dcm_train_data/V2-20240229/no_duplicates/labeling/combined_all_data_labeling_Feb29_final.xlsx"
    )
...
def remove_from_remaining():
    ...
    # Get the indexes of rows in final_df where 'label' is not NaN
    indexes_to_remove = final_df[final_df["label"].notna()].index

    # Remove rows from remaining_df based on indexes
    remaining_df = final_df.drop(indexes_to_remove, errors="ignore")
...
# # Label diffusion related data based on image type components and gradient info
# # adc
# for index, row in df.iterrows():
#     # print(row)
#     if "ADC" in str(row["ImageType"]).upper():
#         if "EADC" in str(row["ImageType"]).upper():
#             df.loc[index, "label"] = "eadc"
#         else:
#             df.loc[index, "label"] = "adc"
#     elif "TRACEW" in str(row["ImageType"]).upper():
#         if row["Diffusionb-valueMax"] == 0:
#             df.loc[index, "label"] = "b0"
#         else:
#             df.loc[index, "label"] = "tracew"
#     elif "FA" in str(row["ImageType"]).upper():
#         if "FAT" not in str(row["ImageType"]).upper():
#             df.loc[index, "label"] = "fa"
#     elif not row.isna()["[DiffusionGradientDirection]"]:
#         df.loc[index, "label"] = "dwig"
#     elif "DIFFUSION" in str(row["ImageType"]).upper():
#         if row["Diffusionb-valueMax"] == 0:
#             df.loc[index, "label"] = "b0"
#
...
for index, row in df.iterrows():
    # print(row)
    if not row.isna()["[DiffusionGradientDirection]"]:
        df.loc[index, "HasDiffusionGradientOrientation"] = 1
    else:
        df.loc[index, "HasDiffusionGradientOrientation"] = 0

    if float(row["Diffusionb-valueMax"]) >= 0:
        df.loc[index, "Diffusionb-valueBool"] = 1
    else:
        df.loc[index, "Diffusionb-valueBool"] = 0
