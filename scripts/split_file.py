...
def identify_and_drop_unusable_cols(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    return input_frame.drop(columns=droppable_cols, errors="ignore")
...
droppable_cols = [
    "list_of_ordered_volume_files" "Unnamed: 0",
    "Collimator Type",
...
input_frame = pd.read_excel(combined_file)
chunk_size = input_frame.shape[0] // 25
i = 1
for chunk in range(0, input_frame.shape[0], chunk_size):
    df = input_frame.iloc[chunk : chunk + chunk_size]
    df.drop(columns=droppable_cols, errors="ignore")
    df.to_excel(
        f"/tmp/dcm_classifier_training_data/dcm_train_data/combined/split_combined_file/combined_iowa_stroke_data_{i}.xlsx",
        index=False,
    )
    i += 1
